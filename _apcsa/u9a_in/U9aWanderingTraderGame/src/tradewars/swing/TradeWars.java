...
public class TradeWars extends javax.swing.JFrame {
  ...
  private void initComponents() {
    ...
    private void makeTradeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_makeTradeActionPerformed
      ...
      try {
        ...
        String logMsg;
        Color c = U9aWanderingTraderGame.TRADE_COLOR;

        if (trade.getMode() == Trade.Mode.BUY) {
          logMsg = String.format("You bought %d \u00D7 %s at %s!",
                  quantity,
                  trade.getProduct().getName(),
                  U9aWanderingTraderGame.CURRENCY_FORMATTER.format(trade.getPrice()));
        } else {
          ...
          if (profit > 0) {
            ...
            c = U9aWanderingTraderGame.PROFITED_COLOR;
          } else {
            profitLabel.setIcon(new javax.swing.ImageIcon(getClass().getResource("/tradewars/resources/minus.png")));
            c = U9aWanderingTraderGame.LOST_COLOR;

          }

          profitLabel.setText(U9aWanderingTraderGame.CURRENCY_FORMATTER.format(profit));
          profitLabel.setVisible(true);

          logMsg = String.format("You sold %d \u00D7 %s for %s, making %s!",
                  quantity,
                  trade.getProduct().getName(),
                  U9aWanderingTraderGame.CURRENCY_FORMATTER.format(salePrice),
                  U9aWanderingTraderGame.CURRENCY_FORMATTER.format(profit));
        }
    ...
    private void stashTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_stashTableMouseClicked
      if (evt.getClickCount() == 2) {
        ...
        long price = 0L;
        try {
          price = U9aWanderingTraderGame.CURRENCY_FORMATTER.parse((String) stashTable.getValueAt(i, 1)).longValue();
    ...
    private void currentLocationActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_currentLocationActionPerformed
      ...
      try {
        if (game.moveTo(l)) {
          logMessage(U9aWanderingTraderGame.MOVED_COLOR, String.format("Day %d: Moved to %s", game.getDay(), l.getName()));
          refreshDisplay();
          reloadProductsTable();
          handleGameEvents();
        }
  ...
  private void refreshDisplay() {
    ...
    timeLeftMeter.setMaximum(U9aWanderingTraderGame.DAYS_IN_GAME);
